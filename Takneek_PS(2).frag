...
// Adding fluctuating colors based on noise and time
vec3 fluctuatingColor(vec2 coord, vec3 baseColor) {
    float fluctuation = noise(coord * 20.0 + u_time * 0.5);  // Adjust fluctuation speed with u_time
    return baseColor + fluctuation * vec3(0.2, 0.1, 0.3);   // Add subtle color variation
}
...
void main(){
    ...
    if(coord.y >0.7){
    vec3 color = vec3(1.0-0.5*sin(u_time)); //background colour
    ...
    if(coord.y<0.7){
        ...
        color = mix(fluctuatingColor(coord,vec3(0.1, 0.3, 0.5)), color, width1);  // First river: blueish color
        
        // Apply second river color
        if (coord.y > y7 && coord.y < y8) {
            color = mix(vec3(1.0, 1.0, 1.0), color, width2);  // Third river: reddish color
        }  // Second river: greenish color
        ...
        float shading1 = smoothstep(0., 1., abs(coord.x - path1));
